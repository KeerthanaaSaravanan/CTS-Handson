...
Each model has a corresponding `ModelAdmin` class that defines how it appears and behaves in the admin.
...
## Django REST Framework (Hands-On 3 - Task 1)

### APIView
`APIView` is a class-based view provided by Django REST Framework (DRF) that allows you to define HTTP method handlers (get, post, put, delete, etc.) as methods of a class. It wraps Django's standard `View` class and provides additional features such as request parsing, response rendering, and authentication handling. In this hands-on, we used `APIView` to create `CourseListView` (for listing and creating courses) and `CourseDetailView` (for retrieving, updating, and deleting a single course).

### Serializer
A `Serializer` in DRF is responsible for converting complex data types (such as Django model instances) and other content types. It also handles deserialization, incoming data. `ModelSerializer` is a shortcut that automatically generates a serializer class with fields corresponding to the model fields. In this exercise, we created `DepartmentSerializer`, `CourseSerializer`, `StudentSerializer`, and `EnrollmentSerializer` using `ModelSerializer` with `fields = '__all__'` to include every model field.

### Request Object
The `request` parameter passed to view methods is an instance of DRF's `Request` class, which extends Django's `HttpRequest`. It provides convenient access to data submitted in the request (e.g., `request.data` for parsed JSON or form data), as well as metadata about the request (headers, user, etc.). In our views, we used `request.data` to obtain the payload sent by the client when creating or updating a course.

### Response Object
The `Response` class in DRF is used to return HTTP responses from API views. Unlike Django's `HttpResponse`, `Response` accepts unserialized data (like Python dictionaries or lists) and automatically renders it to the appropriate content type based on client negotiation (usually JSON). It also allows you to specify the HTTP status code via the `status` argument (imported from `rest_framework import status`). We used `Response` to return serialized data, error messages, and appropriate status codes (200, 201, 204, 400, 404).

For detailed endpoint specifications and testing instructions, see `api_testing.md`.

## Custom DRF Actions (Hands-On 3 - Task 2)

### Why Use Actions?
While `ModelViewSet` provides standard CRUD operations (list, create, retrieve, update, partial_update, destroy), there are times when you need additional endpoints that don't fit the standard CRUD pattern. DRF's `@action` decorator allows you to add custom methods to a ViewSet that are exposed as extra endpoints.

### How DRF Adds Custom Endpoints
When you decorate a method in a ViewSet with `@action`, DRF automatically creates a route for that method based on the method name and the parameters you specify (e.g., `detail=True` for detail routes, `methods=['get']` for HTTP methods). The router then includes these custom routes in the URL configuration alongside the standard ones.

### Example: Enrolled Students in a Course
In this hands-on, we added a custom action to `CourseViewSet`:
- **Endpoint**: `GET /api/courses/{id}/students/`
- **Purpose**: Returns a list of students enrolled in the specified course.
- **Implementation**: The `students` method (decorated with `@action(detail=True, methods=['get'])`) retrieves the course, then queries the `Enrollment` model to find all enrollments for that course, extracts the related students, and returns them serialized with `StudentSerializer`.

For full details on this custom action, including sample requests and responses, see `custom_actions.md`.

--- 

**All work remains inside `handson_03`; `handson_01` and `handson_02` are unchanged.**